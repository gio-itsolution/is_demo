...
from my_best_call_manager.utils.table_creation import get_html_row, get_html_table
from my_best_call_manager.utils.api_methods import *
from integration_utils.bitrix24.bitrix_user_auth.main_auth import main_auth


@main_auth(on_cookies=True)
def my_find_finish_task(request):
    ...
    if request.method == "POST":
        ...
        if not app_tasks_id or app_tasks_id[0] == '':
            messages.error(request, "Нужно сперва поставить задачи, либо нет звонков")
            return render(request, 'find_tasks.html')
        ...
        if not completed_tasks:
            messages.error(request, "There's no completed tasks")
            return render(request, 'find_tasks.html')
        ...
        post_id = add_post(but, f"{html.unescape(html_table)}", [f"SG{group_id}"])

        context_data = {'post_id': post_id,
                        'user_id': but.id,
                        'completed_tasks': completed_tasks}

        return render(request, "find_tasks.html", context=context_data)
